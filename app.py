...
:root {
    --bg-primary: #0D1B2E; --bg-secondary: #112240; --bg-card: #1A2F4A; --bg-card-hover: #1E3550;
    --border: rgba(255,255,255,0.08); --border-accent: rgba(59,130,246,0.4);
    --text-primary: #E8F0FE; --text-secondary: #C8C8C8; --text-tertiary: #C8C8C8;
    --accent-blue: #4F9EFF; --accent-amber: #FFB020; --accent-green: #3DFFA0; --accent-red: #FF5A5A;
...
[data-testid="stForm"] button { background: #3B82F6 !important; color: white !important; font-weight: 700 !important; border-radius: 10px !important; font-size: 1rem !important; letter-spacing: 0.02em !important; }
[data-testid="stForm"] button:hover { background: #2563EB !important; transform: translateY(-1px) !important; }
...
def load_data():
    ...
    ateneo_regione = {
        ...
        'Università di Genova': ('Liguria', 'Liguria'), 'Università di Milano': ('Lombardia', 'Lombardia'),
        'Bicocca': ('Lombardia', 'Lombardia'), 'San Raffaele': ('Lombardia', 'Lombardia'),
        "Università dell'Insubria": ('Lombardia', 'Lombardia'), 'Università di Brescia': ('Lombardia', 'Lombardia'),
        'Università di Pavia': ('Lombardia', 'Lombardia'), 'Università di Trento': ('Trentino-Alto Adige', 'Trentino-Alto Adige/Südtirol'),
        'Università di Verona': ('Veneto', 'Veneto'), 'Università di Padova': ('Veneto', 'Veneto'),
        ...
        'Università di Parma': ('Emilia-Romagna', 'Emilia-Romagna'), 'Università di Ferrara': ('Emilia-Romagna', 'Emilia-Romagna'),
        'Università di Firenze': ('Toscana', 'Toscana'), 'Università di Pisa': ('Toscana', 'Toscana'),
        'Università di Siena': ('Toscana', 'Toscana'), 'Università di Perugia': ('Umbria', 'Umbria'),
        'Università di Camerino': ('Marche', 'Marche'), 'Carlo Bo': ('Marche', 'Marche'),
        'La Sapienza': ('Lazio', 'Lazio'), 'Tor Vergata': ('Lazio', 'Lazio'),
        'Università della Tuscia': ('Lazio', 'Lazio'), "Università de L'Aquila": ('Abruzzo', 'Abruzzo'),
        'Università di Teramo': ('Abruzzo', 'Abruzzo'), 'Università del Sannio': ('Campania', 'Campania'),
        'Federico II': ('Campania', 'Campania'), 'Università Vanvitelli': ('Campania', 'Campania'),
        'Università di Bari': ('Puglia', 'Puglia'), 'Università del SALENTO': ('Puglia', 'Puglia'),
        'Università di Foggia': ('Puglia', 'Puglia'), 'Università della Basilicata': ('Basilicata', 'Basilicata'),
        'Magna Graecia': ('Calabria', 'Calabria'), 'Università della Calabria': ('Calabria', 'Calabria'),
        'Università di Palermo': ('Sicilia', 'Sicilia'), 'Università di Catania': ('Sicilia', 'Sicilia'),
        'Università di Messina': ('Sicilia', 'Sicilia'), 'Università di Sassari': ('Sardegna', 'Sardegna'),
        'Università di Cagliari': ('Sardegna', 'Sardegna'),
    }
    regione_macro = {
        'Piemonte': 'Nord', 'Lombardia': 'Nord', 'Veneto': 'Nord', 'Friuli-Venezia Giulia': 'Nord',
        'Liguria': 'Nord', 'Emilia-Romagna': 'Nord', 'Trentino-Alto Adige': 'Nord',
        'Toscana': 'Centro', 'Umbria': 'Centro', 'Marche': 'Centro', 'Lazio': 'Centro', 'Abruzzo': 'Centro',
        'Campania': 'Sud', 'Puglia': 'Sud', 'Basilicata': 'Sud', 'Calabria': 'Sud', 'Molise': 'Sud',
        'Sicilia': 'Isole', 'Sardegna': 'Isole',
    }
...
def fonte_annotation(testo):
    return dict(x=0.99, y=-0.13, xref='paper', yref='paper', text=testo, showarrow=False,
                font=dict(size=10, color='#7A9CC0'), align='right', xanchor='right')
...
with st.sidebar:
    ...
    <div style='padding: 1.5rem 0 1rem 0;'>
        <div style='font-size:1.1rem; font-weight:600; color:#F5F5F7; letter-spacing:-0.02em;'>L-2 Biotecnologie</div>
...
# ─── AVVII DI CARRIERA ───────────────────────────────────────────────────────
    ...
    for i, anno in enumerate(anni_tree_avvi):
        ...
        fig_tree_avvi.add_trace(go.Treemap(
            labels=subset['corso_norm'], parents=['L-2 Biotecnologie']*len(subset),
            ...
            marker=dict(colorscale=[[0,'#1E3A5F'],[0.4,'#2563EB'],[0.7,'#60A5FA'],[1,'#BAE6FD']],
                colors=subset['n_atenei'], colorbar=dict(title=dict(text='N° atenei', font=dict(color='#9CA3AF')), tickfont=dict(color='#9CA3AF')),
    ...
    fig_tree_avvi.update_layout(
        ...
        updatemenus=[dict(type='buttons', direction='right', x=0.5, xanchor='center', y=1.10, yanchor='top',
            buttons=buttons_tree_avvi, bgcolor='#1F2937', bordercolor='#3B82F6', borderwidth=1,
    ...
    palette_lazio = {'La Sapienza': ['#F59E0B','#FCD34D','#F97316'], 'Tor Vergata': ['#3B82F6'], 'Università della Tuscia': ['#34D399']}
    colori_label_avvi = {}
...
# ─── PROFILO STUDENTI ─────────────────────────────────────────────────────────
    ...
    indicatori_g5 = [('pct_soddisfatti','Soddisfatti del corso','#3B82F6'),('pct_riiscrizione','Si reiscriverebbero','#34D399'),('pct_magistrale','Prosegue magistrale','#818CF8')]
    val_2025 = {'pct_soddisfatti': 90.8, 'pct_riiscrizione': 71.7, 'pct_magistrale': 88.7}
    fig5 = go.Figure()
        ...
        for col_idx, (col, titolo, colore) in enumerate(indicatori_g5):
            ...
            fig5.add_trace(go.Indicator(mode='gauge+number+delta', value=val_2025[col],
                ...
                gauge={'axis': {'range': [0,100], 'ticksuffix': '%', 'tickfont': {'color': '#6B7280'}, 'tickcolor': '#C8C8C8'},
                    'bar': {'color': colore, 'thickness': 0.25}, 'bgcolor': '#1F2937', 'borderwidth': 0,
    ...
    fig5.update_layout(
        title=dict(text='Profilo laureati L-2 Biotecnologie — 2025 vs 2024', font=dict(size=20, color='white', family='Arial'), x=0.5, xanchor='center'),
    ...
    DEST = {'pct_stesso_ateneo_magistrale': ('Stesso ateneo','#3B82F6'), 'pct_magistrale_nord': ('Altro Nord','#818CF8'), 'pct_magistrale_centro': ('Centro Italia','#F59E0B'), 'pct_altro': ('Estero / Altro','#C8C8C8')}
    fig6 = go.Figure()
...
# ─── PERCORSO ACCADEMICO ──────────────────────────────────────────────────────
    ...
    # G2 — Laureati
    chart_header("Laureati L-2 Biotecnologie — Italia (2010–2024)",
        ...
        "Passa il cursore sulle barre per vedere il valore esatto.")
    fig2 = px.bar(lau_naz, x='AnnoS', y='Lau', color='COVID', color_discrete_map={True:'#EF4444', False:'#3B82F6'}, labels={'Lau':'N° Laureati','AnnoS':'Anno'}, text='Lau')
    fig2.update_traces(texttemplate='%{text:,}', textposition='outside', textfont=dict(size=9, color='#9CA3AF'), marker_line_width=0, opacity=0.9)
...
# ─── ANALISI AVANZATA ─────────────────────────────────────────────────────────
    ...
    fig_radial.update_layout(font=dict(family='Inter', size=12), plot_bgcolor=BG_PAPER, paper_bgcolor=BG_PAPER,
        title=dict(text='Studenti al II Anno con ≥ 2/3 CFU — iC16bis L-2 Biotecnologie', font=dict(size=16, color='white', family='Inter'), x=0.5, xanchor='center'),
        showlegend=True, legend=dict(font=dict(color='#D1D5DB', size=11), bgcolor='rgba(0,0,0,0)', orientation='v', x=1.02, y=0.5, xanchor='left'),
    ...
    fig_corr.update_layout(**PLOT_LAYOUT, title='',
        annotations=[
            dict(x=0.98, y=0.98, xref='paper', yref='paper', text=f'r = {r:.3f} · p = {p:.3f} · n = {len(df_plot_c)}',
                 showarrow=False, font=dict(size=12, color='#9CA3AF'), align='right', xanchor='right', bgcolor='#1F2937', bordercolor='#C8C8C8', borderwidth=1),
...
# ─── SINTESI ──────────────────────────────────────────────────────────────────
    ...
    fig7.update_layout(title=dict(text='Scorecard L-2 Biotecnologie',font=dict(size=18,color='white',family='Inter'),x=0.5,xanchor='center'),
